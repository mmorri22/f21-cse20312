...
class min_heap{
	...
	public:
		...
		// Copy Constructor
		min_heap( const min_heap<T>& copy ) : heap_vec( copy ) {}
		
		// Assignment Operator
		min_heap& operator=( const min_heap<T>& assign ){
		...
		friend std::ostream& operator<<( std::ostream& out, const min_heap<T>* print_heap ){
